...
Minimal ESP-IDF firmware for verifying remote GDB debugging on the testbench. Prints a counter to serial and exposes a `debug_loop()` function with a global `loop_counter` variable for GDB breakpoint and variable inspection testing.
...
The testbench auto-assigns serial ports. Query `/api/devices` to find the port, then flash:

```bash
# Find the auto-assigned port
PORT=$(curl -s http://testbench.local:8080/api/devices | \
